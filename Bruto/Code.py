...
# criar e configurar um botão, textos e outros elementos
...
def converter_moeda():
    ...
    if moeda_origem and moeda_destino:
        ...
        texto_cotação_moeda.configure(text=f'1 {moeda_origem} = {cotacao} {moeda_destino}')


botao_converter = customtkinter.CTkButton(janela, text='Converter', command=converter_moeda)
lista_moedas = customtkinter.CTkScrollableFrame(janela)
texto_cotação_moeda = customtkinter.CTkLabel(janela, text='')

moedas_disponiveis = nomes_moedas()

for moeda in moedas_disponiveis:
    nome_moeda = moedas_disponiveis[moeda]
    texto_moeda = customtkinter.CTkLabel(lista_moedas, text=f'{moeda}: {nome_moeda}')
    texto_moeda.pack()
...
texto_cotação_moeda.pack(padx=10, pady=10)
